...
import { View, Text, TouchableOpacity, StyleSheet, ScrollView, Image, Animated, TextInput, Alert, Dimensions, Share, Platform } from 'react-native';
  ...
  return (
    <ScrollView style={[s.container, { backgroundColor: theme.background }]} contentContainerStyle={{ padding:20, paddingBottom:40 }}>
      ...
      <View style={{ flexDirection:'row', marginTop:8, gap: 8 }}>
        <TouchableOpacity style={[s.shareBtn, { backgroundColor: '#3b82f6' }]} onPress={async () => {
          try {
            await Share.share({
              message: `AGBOT Scan Report\n\nPest: ${pest}\nConfidence: ${Math.round(conf)}%\nSeverity: ${sev}\n${result.damage_pattern || ''}\n\nScanned with AGBOT - AI Plant Pest Detection`,
            });
          } catch(e) {}
        }}>
          <Ionicons name="share-outline" size={18} color="#fff" />
          <Text style={{ color:'#fff', fontWeight:'700', marginLeft:6, fontSize:13 }}>Share</Text>
        </TouchableOpacity>

        <TouchableOpacity style={[s.shareBtn, { backgroundColor: '#8b5cf6' }]} onPress={async () => {
          try {
            const html = `
              <html><head><style>
                body{font-family:system-ui;padding:30px;color:#1f2937}
                h1{color:#16a34a;margin-bottom:5px} h2{color:#374151;font-size:18px}
                .badge{display:inline-block;padding:4px 14px;border-radius:12px;font-weight:700;font-size:13px;color:white;background:${isHealthy?'#10b981':'#ef4444'}}
                .section{background:#f9fafb;border:1px solid #e5e7eb;border-radius:12px;padding:16px;margin:12px 0}
                .section h3{margin:0 0 8px;font-size:15px} li{margin-bottom:4px;font-size:14px}
                .conf-bar{height:10px;background:#e5e7eb;border-radius:5px;margin:8px 0}
                .conf-fill{height:10px;border-radius:5px;background:${conf>80?'#10b981':'#f59e0b'}}
              </style></head><body>
              <h1>AGBOT</h1><p style="color:#6b7280">AI Plant Pest Detection Report</p><hr>
              <h2>${isHealthy ? 'Plant is Healthy!' : pest}</h2>
              ${!isHealthy && sci ? '<p style="color:#6b7280;font-style:italic">('+sci+')</p>' : ''}
              <p><span class="badge">${sev}</span></p>
              <p><strong>Confidence:</strong> ${Math.round(conf)}%</p>
              <div class="conf-bar"><div class="conf-fill" style="width:${conf}%"></div></div>
              ${result.damage_pattern ? '<div class="section"><h3>Description</h3><p>'+result.damage_pattern+'</p></div>' : ''}
              ${symptoms.length ? '<div class="section"><h3>Symptoms</h3><ul>'+symptoms.map(s=>'<li>'+s+'</li>').join('')+'</ul></div>' : ''}
              ${tx.immediate?.length ? '<div class="section"><h3>Immediate Actions</h3><ul>'+tx.immediate.map(s=>'<li>'+s+'</li>').join('')+'</ul></div>' : ''}
              ${chem.length ? '<div class="section"><h3>Chemical Treatments</h3><ul>'+chem.map(s=>'<li>'+s+'</li>').join('')+'</ul></div>' : ''}
              ${org.length ? '<div class="section"><h3>Organic Treatments</h3><ul>'+org.map(s=>'<li>'+s+'</li>').join('')+'</ul></div>' : ''}
              ${prev.length ? '<div class="section"><h3>Prevention</h3><ul>'+prev.map(s=>'<li>'+s+'</li>').join('')+'</ul></div>' : ''}
              <hr><p style="color:#9ca3af;font-size:12px">Generated by AGBOT on ${new Date().toLocaleDateString()}</p>
              </body></html>`;
          ...
          <Text style={{ color:'#fff', fontWeight:'700', marginLeft:6, fontSize:13 }}>PDF Report</Text>
        </TouchableOpacity>
      </View>
...
const s = StyleSheet.create({
  ...
  shareBtn:{flex:1,flexDirection:'row',alignItems:'center',justifyContent:'center',height:44,borderRadius:12},
